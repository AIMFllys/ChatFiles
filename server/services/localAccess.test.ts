...
test('maps six public operations to bounded shared read-only tools', async () => {
  ...
  await service.conversations({ query: '中文', limit: 999 })
  await service.search({ query: '目标', conversationId: 'conv-a', limit: 0 })
  await service.artifacts({ query: '文档', category: 'document', limit: 20 })
  await service.readDocument({ assetId: 'a'.repeat(64), maxCharacters: 999_999 })
  await service.messageContext({ messageUid: 'm-1', radius: 999 })
