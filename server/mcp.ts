...
const limit = z.number().int().min(1).max(100).default(20).describe('最多返回 1–100 条记录')
...
export function createChatFilesMcpServer(service: LocalAccessService = createRuntimeLocalAccessService(
  ...
  server.registerTool('chatfiles_list_conversations', {
    ...
    inputSchema: z.object({
      query: z.string().max(120).optional().describe('可选会话名称'), limit, response_format: responseFormat,
  ...
  server.registerTool('chatfiles_search_messages', {
    ...
    inputSchema: z.object({
      query: z.string().min(1).max(500), conversation_id: z.string().max(512).optional(),
      sender: z.string().max(512).optional(), limit, response_format: responseFormat,
    }).strict(), outputSchema, annotations,
  ...
  server.registerTool('chatfiles_search_artifacts', {
    ...
    inputSchema: z.object({
      query: z.string().max(200).optional(), conversation_id: z.string().max(512).optional(),
      category: z.enum(['all', 'work', 'document', 'skill', 'link']).default('all'),
      limit, response_format: responseFormat,
    }).strict(), outputSchema, annotations,
  ...
  server.registerTool('chatfiles_read_document', {
    ...
    inputSchema: z.object({
      asset_id: z.string().regex(/^[0-9a-f]{64}$/u),
      max_characters: z.number().int().min(1).max(50_000).default(12_000),
  ...
  server.registerTool('chatfiles_get_message_context', {
    ...
    inputSchema: z.object({
      message_uid: z.string().min(1).max(512), radius: z.number().int().min(0).max(20).default(8),
      response_format: responseFormat,
    }).strict(), outputSchema, annotations,
