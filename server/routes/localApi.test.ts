...
test('serves bounded UTF-8 collections and rejects unknown parameters', async () => {
  ...
  await withServer(router, async (baseUrl) => {
    const response = await fetch(`${baseUrl}/api/local/v1/conversations?query=${encodeURIComponent('中文')}&limit=999`)
    assert.equal(response.status, 200)
...
test('exposes artifacts, bounded document text, and message context through stable ID routes', async () => {
  await withServer(createLocalApiRouter({ service: service() }), async (baseUrl) => {
    ...
    const document = await fetch(`${baseUrl}/api/local/v1/documents/${'a'.repeat(64)}?maxChars=999999`)
    const context = await fetch(`${baseUrl}/api/local/v1/messages/${encodeURIComponent('消息-一')}/context?radius=999`)
