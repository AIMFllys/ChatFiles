...
本项目是**通用模板**：文档里用机主（"{{机主昵称}}"，{{机主身份}}）的真实数字作为**范例与默认值**，所有机器/账号相关值用 `{{占位符}}` 标出，替换为自己的即可。除你自己的聊天/媒体内容外，全部功能都能凭本包复刻。两条路径：**A. 一键喂给 AI**（[`docs/PROMPT.md`](docs/PROMPT.md)）/ **B. 工程师手动**（[`docs/RUNBOOK.md`](docs/RUNBOOK.md)）。
